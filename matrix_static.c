...
matrix transpose(matrix m1)
{
	...
	int j = 0;
	matrix result;
	if (m1.row_dim == m1.col_dim){
		result = create_empty(m1.row_dim, m1.col_dim);
		for (i = 0; i < m1.row_dim; i++)
			for (j = 0; j < m1.col_dim; j++)
				result.element[i][j] = m1.element[j][i];
				printf("\nInside of the Transpose function, the function has noticed the rows and columns are equal; here is your transposed matrix.");
				matrix_print(result);
	}
	return result;
}
